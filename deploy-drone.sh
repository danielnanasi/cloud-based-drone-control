...
for i in $(seq 1 ${NUMBER_OF_DRONES}); do
    ...
    echo "docker run --name drone-$i -e MASTER_IP=${MASTER_IP} -e MAVLINK_PORT=${MAVLINK_NODE_PORT} -p ${SIMULATION_PORT}:10000 -p ${FCU_PORT}:14580/udp -d nanasidnl/drone_control:px4sim"
    docker run --name drone-$i -e MASTER_IP=${MASTER_IP} -e MAVLINK_PORT=${MAVLINK_NODE_PORT} -p ${SIMULATION_PORT}:10000 -p ${FCU_PORT}:14580/udp -d nanasidnl/drone_control:px4sim
